...
# See this if hatches (patterns) are not showing up: https://stackoverflow.com/a/59389823
PATTERNS = ['.', '/', '.O']
...
def plot_bookings_per_week(per_week_results: dict[int, list[int]]) -> None:
    ...
    for i, year in enumerate(sorted(per_week_results)):
        ...
        rects = plt.bar(x_values - (bar_width/2) + i*bar_width,
                        year_counts, bar_width, label=year, color=COLORS[i], hatch=PATTERNS[i], alpha=0.99)
